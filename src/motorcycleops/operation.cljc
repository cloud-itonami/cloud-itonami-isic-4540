(ns motorcycleops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (MotorcycleOpsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the MotorcycleOpsGovernor
  ...
  human dealership/workshop coordinator. The approver resumes with
...
(defn build
  ...
  `motorcycleops.store/Store`).
  opts:
    :advisor      -- a `motorcycleops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MotorcycleOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; MotorcycleOpsGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
