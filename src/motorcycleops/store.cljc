(ns motorcycleops.store
  "SSoT for the ISIC-4540 SALE, MAINTENANCE AND REPAIR OF MOTORCYCLES AND
  RELATED PARTS AND ACCESSORIES operations-coordination actor, behind a
  `Store` protocol so the backend is a swap, not a rewrite -- the same
  seam every `cloud-itonami-isic-*` actor in this fleet uses.

  This actor coordinates the back-office operations of a motorcycle
  dealership/workshop (sale/repair-order and parts-used logging, bay/
  technician service-scheduling proposals, safety-concern flagging
  [defects, recalls, unsafe-repair doubts], and parts/inventory
  procurement coordination). It NEVER directly finalizes a
  roadworthiness-clearance decision (certifying a motorcycle safe to
  return to the road after service) -- see `motorcycleops.governor`'s
  `scope-exclusion-violations`, a HARD, permanent, un-overridable
  block. Motorcycle repair/maintenance has a direct road-safety
  dimension: this actor coordinates the back office around that
  ...
  attempt). Each account represents a dealership/workshop customer or
  fleet account under which sale/repair-order, service-scheduling,
  safety-concern, and parts-order proposals are raised.

  A registered/verified sale/repair-order account must exist before
  ANY proposal for it may ever commit or escalate --
  `motorcycleops.governor`'s `account-unverified-violations` re-derives
  this from the account's own `:registered?`/`:verified?` fields,
...
(defprotocol Store
  (account [s account-id] "Registered sale/repair-order account, or nil.
    Account map: {:account-id .. :name .. :registered? bool :verified? bool}.")
  (all-accounts [s])
  (ledger [s] "the append-only immutable decision-fact log")
  (service-log [s] "the append-only committed service-record history")
...
(defn demo-data
  ...
  []
  {:accounts
   {"account-1" {:account-id "account-1" :name "Kanda Motorcycle Works -- Branch 1 (service account)"
                 :registered? true :verified? true}
    "account-2" {:account-id "account-2" :name "Shibuya Riders Garage -- Branch 2 (service account)"
                 :registered? true :verified? true}
...
(defrecord MemStore [a]
  ...
  (commit-record! [_ record]
    (swap! a update :service-log conj record)
    record)
...
(defn seed-db
  ...
  default."
  []
  (->MemStore (atom (assoc (demo-data) :ledger [] :service-log []))))
...
(defn mem-store
  ...
  [accounts]
  (->MemStore (atom {:accounts (or accounts {}) :ledger [] :service-log []})))
