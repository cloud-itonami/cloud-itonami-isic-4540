(ns motorcycleops.store-contract-test
  "Contract tests for `motorcycleops.store/Store` protocol."
...
(deftest mem-store-account-lookup
  (testing "MemStore can store and retrieve accounts by ID (string keys)"
    (let [accounts {"a1" {:account-id "a1" :name "Alice's Workshop" :registered? true :verified? true}}
...
(deftest mem-store-all-accounts
  (testing "MemStore returns all accounts in sorted order"
    (let [accounts {"a2" {:account-id "a2" :name "Bob's Workshop"}
                    "a1" {:account-id "a1" :name "Alice's Workshop"}
                    "a3" {:account-id "a3" :name "Carol's Workshop"}}
          s (store/mem-store accounts)
...
(deftest mem-store-service-log
  (testing "MemStore commit-record! appends to service-log"
    (let [s (store/mem-store {})
          record {:op :log-service-record :account-id "a1" :value {:motorcycle-id "MC-1"}}]
      (is (= 0 (count (store/service-log s))))
      (store/commit-record! s record)
      (is (= 1 (count (store/service-log s))))
      (is (= record (first (store/service-log s)))))))

(deftest mem-store-with-accounts
  (testing "MemStore with-accounts replaces the account directory"
    (let [s (store/mem-store {})
          new-accounts {"a1" {:account-id "a1" :name "Alice's Workshop"}}]
